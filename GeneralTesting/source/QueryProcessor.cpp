    ...
    if (!T) {
        if (para1IsNum) {
            ...
            if (para1.compare(para2) == 0) {
				...
				#else
				Threading threading;
				if (!threading.processNextBipSameVarDriver(temp, para1Val, pkb)) return -1;
				#endif
				...
				#else
				Threading threading;
				if(!threading.processNextBipDiffVarDriver(toStoreTuple, para1ValString, para1ValInt, para2ValString, para2ValInt, isPara1, pkb)) return -1;
				#endif
...
int QueryProcessor::evaluateAffects(bool T, bool para1IsNum, bool para1IsPlaceholder, bool para2IsNum, bool para2IsPlaceholder, std::string para1, std::string para2, int para1Num, int para2Num, PKB& pkb) {
    ...
    if (!T) {
        ...
        // Affects(_, a)
			...
			#else
			Threading threading;
			if (!threading.processAffectsPara1IsPlaceholderDriver(temp, temp2, pkb)) return -1;
			toStore = intVecToStringVec(temp);
			#endif
        ...
        // Affects(a, _)
			...
			#else
			Threading threading;
			if (!threading.processAffectsPara2IsPlaceholderDriver(temp, temp2, pkb)) return -1;
			toStore = intVecToStringVec(temp);
			#endif
        ...
        // Affects*(_, a)
			...
			#else
			Threading threading;
			if (!threading.processAffectsTPara1IsPlaceholderDriver(temp, temp2, pkb)) return -1;
			toStore = intVecToStringVec(temp);
			#endif
        ...
        // Affects*(a, _)
			...
			#else
			Threading threading;
			if (!threading.processAffectsTPara2IsPlaceholderDriver(temp, temp2, pkb)) return -1;
			toStore = intVecToStringVec(temp);
			#endif
			...
			*/
            ...
            // Evaluates with query
                ...
                if (temp.getName().compare("attrCompare_attrRef") == 0) {
                    ...
                    if (tree[tempIndex].getName().compare("ref_attrRef") == 0) {
                    ...
                    else if (tree[tempIndex].getName().compare("ref_synonym") == 0) {
                        std::string syno = (tree[tree[tempIndex].getChildren()[0]]).getName();
